...
客户端的“本地只读、无交易入口、历史分红不是未来承诺、草案暂锁定”边界是正确的；但当前版本**不能作为候选研究结果的发布版本**。主要阻断来自分红重复计数、策略通道过滤错误、发布包/策略版本绑定不足以及桌面安装包缺少运行时自举。
...
### 4. 策略运行未固定首页刚展示的 release

`App.tsx:42-50` 调用 `run_strategy` 时不传 `release_id`，后端缺省读取执行时最新指针。若后台刚发布新 release，用户会看到 A、实际运行 B。

建议：Dashboard 的 release_id 必须进入页面状态并传入运行请求；后端对 release、策略和规则哈希做一致性校验。
...
先修复事实唯一性并重发 release，再修复候选通道过滤和覆盖口径；随后补 runtime bootstrap、release/PIT 绑定和端到端回归，最后才继续开发组合策略与桌面端高级能力。
