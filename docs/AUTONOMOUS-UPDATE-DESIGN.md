...
运行时数据库已部署到 `~/Library/Application Support/HighDividend/data/database/high_dividend.db`。当前发布版本包含5,422只股票、1,603只ETF的行情快照，1,716个标的的历史日线（2019-01-02至2026-08-07），以及4,913只股票的分红事件和5,201只股票的财务观察值。历史日线中达到约五年门槛的股票为1,545/5,422；ETF分配/产品事实仍明显不足，覆盖矩阵会明确显示。工作区中的 `data/database/high_dividend.db` 仍是开发种子库；本机定时任务读取运行时数据库。
